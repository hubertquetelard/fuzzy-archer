...
[MeteoR_OI]
    ...
    [[Display]]
        etp         = {{ display.etp|default('disable') }}
        radiation   = {{ display.radiation|default('disable') }}
        uv          = {{ display.uv|default('disable') }}
...
[BootstrapLabels]
    ...
    title = "{{ location|default('Nouvelle station') }} - M&eacute;t&eacute;oR-OI"
    location_href = "http://obs.meteor-oi.re/{{ station_id|default('XXX000') }}/"
    footer = "&copy; M&eacute;t&eacute;oR-OI 2015-2023  -  Les donn&eacute;es de ce site ne peuvent garantir la protection des biens et des personnes"

    # Variables specifiques a MeteoR-OI (utilisees par les scripts PHP de comparaison)
    station_id      = {{ station_id|default('XXX000') }}
    premier_jour    = {{ data_start.day|default('1') }}
    premier_mois    = {{ data_start.month|default('1') }}
    premiere_annee  = {{ data_start.year|default('2019') }}
    sonde_sol       = {{ modules.radiation|default('FALSE') }}
...
{% if display.radiation == "enable" %}
    [[max_radiation]]
        ...
        colours =  "#40FF00", "#80FF00", "#BFFF00", "#FFFF00", "#FFBF00", "#FF8000", "#FF4000", "#FF0000"
{% endif %}
{% if display.etp == "enable" %}
    [[etp]]
        obs_type = ET
        aggregate_type = sum
