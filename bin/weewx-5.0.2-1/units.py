...
def as_value_tuple_for_agg(accumulator_dict, obs_type, aggregate_type):
    ...
    # Is the accumulator None?
    if accumulator_dict is None:
        # Yes. The accumulator was probably None. Signal a value of None
        ...
        # There is an accumulator, and the observation type is in it.
        if aggregate_type == 'min':
            val = accumulator_dict[obs_type].min
        elif aggregate_type == 'mintime':    
            val = accumulator_dict[obs_type].mintime
        elif aggregate_type == 'max':
            val = accumulator_dict[obs_type].max
        elif aggregate_type == 'maxtime':
            val = accumulator_dict[obs_type].maxtime
        elif aggregate_type == 'sum':
            val = accumulator_dict[obs_type].sum
        elif aggregate_type == 'wsum':
            val = accumulator_dict[obs_type].wsum
        elif aggregate_type == 'sumtime':
            val = accumulator_dict[obs_type].sumtime
        elif aggregate_type == 'count':
            val = accumulator_dict[obs_type].count
        elif aggregate_type == 'last':
            val = accumulator_dict[obs_type].last
            #  to extract wind last val from tuple
            if obs_type == 'wind':
                val = val[0]
